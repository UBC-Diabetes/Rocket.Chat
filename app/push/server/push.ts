...
export const isFCMCredentials = ajv.compile<FCMCredentials>(
    FCMCredentialsValidationSchema
);
...
export type NativeNotificationParameters = {
    ...
    _replaceToken: (
        currentToken: IAppsTokens["token"],
        newToken: IAppsTokens["token"]
    ) => void;
...
class PushClass {
    ...
    public configure(options: PushOptions): void {
        ...
        if (this.options.apn) {
            initAPN({
                options: this.options as RequiredField<PushOptions, "apn">,
        ...
        if ("apn" in app.token && app.token.apn) {
            ...
            if (this.options.apn) {
                sendAPN({
                    ...
                    notification: { topic: app.appName, ...notification },
            ...
            if (!useLegacyProvider) {
                ...
                sendFCM({
                    ...
                    _removeToken: this.removeToken,
                    options: sendGCMOptions as RequiredField<
                        PushOptions,
                        "gcm"
                    >,
                });
    ...
    private async getNativeNotificationAuthorizationCredentials(): Promise<{
        token: string;
        projectId: string;
    }> {
        const credentialsString = settings.get<string>(
        ...
        try {
            ...
            if (!isFCMCredentials(credentials)) {
                throw new Error(
                    "Push_google_api_credentials is not in the correct format"
                );
            }
    ...
    private async sendGatewayPush(
        ...
        logger.error({
            msg: `Error sending push to gateway (${tries} try) ->`,
            err: response,
        });
    ...
    private getGatewayNotificationData(
        notification: PendingPushNotification
    ): Omit<GatewayNotification, "uniqueId"> {
    ...
    private async sendNotificationGateway(
        ...
        const gatewayNotification =
            this.getGatewayNotificationData(notification);
    ...
    private async sendNotification(
        ...
        for await (const app of appTokens) {
            ...
            if (this.shouldUseGateway()) {
                await this.sendNotificationGateway(
                    app,
                    notification,
                    countApn,
                    countGcm
            ...
            await this.sendNotificationNative(
                app,
                notification,
                countApn,
                countGcm
        ...
        if (settings.get("Log_Level") === "2") {
            ...
            if (!countApn.length && !countGcm.length) {
                if ((await AppsTokens.col.estimatedDocumentCount()) === 0) {
                    logger.debug(
                        'GUIDE: The "AppsTokens" is empty - No clients have registered on the server yet...'
                    );
                }
            } else if (!countApn.length) {
                if ((await AppsTokens.countApnTokens()) === 0) {
                    logger.debug(
                        'GUIDE: The "AppsTokens" - No APN clients have registered on the server yet...'
                    );
                }
            } else if (!countGcm.length) {
                if ((await AppsTokens.countGcmTokens()) === 0) {
                    logger.debug(
                        'GUIDE: The "AppsTokens" - No GCM clients have registered on the server yet...'
                    );
                }
            }
        }
    ...
    private hasApnOptions(
        options: IPushNotificationConfig
    ): options is RequiredField<IPushNotificationConfig, "apn"> {
        return Match.test(options.apn, Object);
    }

    private hasGcmOptions(
        options: IPushNotificationConfig
    ): options is RequiredField<IPushNotificationConfig, "gcm"> {
    ...
    public async send(options: IPushNotificationConfig) {
        ...
        try {
            ...
            logger.debug(
                `Could not send notification to user "${notification.userId}", Error: ${error.message}`
            );
